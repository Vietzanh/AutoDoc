...
# Logical layout block aggregating multiple text elements undera single layout region
@dataclass
class LayoutBlock:
...
# This groups spans by their parent block, which has a better bbox for matching
def text_blocks_from_pdf_elements(pdf_elements: Iterable[Dict[str, Any]]) -> List[TextBlock]:
    ...
    for elem in pdf_elements:
        ...
        # Convert all spans in this block to TextElement objects
        spans = elem.get("spans", [])
        span_elements = text_elements_from_spans(spans)
        
        # Get block text (use content if available, otherwise join spans)
...
# Convert YOLO detections (already mapped to PDF coordinates) into LayoutRegion objects
...
# Assign each text element to the best-overlapping layout region (if any) using IoU, and aggregate them into LayoutBlock objects
# DEPRECATED: Use match_blocks_to_layout instead for better IoU matching
def match_elements_to_layout(
    ...
    # Prepare containers
    region_assignments: Dict[int, List[TextElement]] = {
        i: [] for i in range(len(layout_regions))
    }

    # For each text element, find the best region by IoU
    for elem in text_elements:
        best_idx = None
        best_iou = 0.0
        for idx, region in enumerate(layout_regions):
            overlap = iou(elem.bbox, region.bbox)
            if overlap > best_iou:
                best_iou = overlap
                best_idx = idx
        if best_idx is not None and best_iou >= iou_threshold:
            region_assignments[best_idx].append(elem)

    # Build LayoutBlocks
    blocks: List[LayoutBlock] = []
    for idx, region in enumerate(layout_regions):
        elems = region_assignments[idx]
        if not elems:
            continue

        # Sort by reading order: top-to-bottom, left-to-right
        elems_sorted = sorted(elems, key=lambda e: (e.bbox[1], e.bbox[0]))
        joined_text = " ".join(e.text.strip() for e in elems_sorted if e.text.strip())
...
def match_blocks_to_layout(
    ...
    This gives better IoU scores than matching individual spans.
    ...
    # Prepare containers: each region can have multiple blocks assigned
    region_assignments: Dict[int, List[TextBlock]] = {
        i: [] for i in range(len(layout_regions))
    }

    # For each text block, find the best matching region by IoU
    ...
    for idx, region in enumerate(layout_regions):
        ...
        # Collect all spans from all matched blocks with their block indices and original order
        all_spans_with_block_idx: List[Tuple[TextElement, int, int]] = []
        # (span, block_idx, original_span_idx_in_block)
        ...
        # Extract sorted spans and create mapping
        ...
        # Also store original span order within each block for proper ordering
        ...
        layout_block = LayoutBlock(
            ...
            extra={
                ...
                "span_to_block_idx": sorted_span_to_block_idx,  # Maps element index to text block index
                "span_to_original_order": sorted_span_to_original_order,  # Maps element index to (block_idx, original_span_idx)
            },
        )
