...
def process_text_block(
    ...
    is_centered = hasattr(block, 'extra') and block.extra.get("is_centered", False)
    ...
    # For heading blocks: split elements into heading text vs. body text
    # This fixes cases where YOLO groups "References" and "[1] ..." into one block
    heading_elements = block.elements
    overflow_elements = []
    if style_name.startswith("Heading") and len(block.elements) > 1:
        # Find where the heading text ends — heading is typically the first visual line
        first_elem = block.elements[0]
        # Use the vertical center of the first element to check for new lines, 
        # since bbox bottoms can overlap due to line-height padding
        heading_y_center = first_elem.bbox[1] + (first_elem.bbox[3] - first_elem.bbox[1]) * 0.5
        
        heading_elems = [first_elem]
        body_elems = []
        for e in block.elements[1:]:
            # If the element's top is below the vertical center of the heading line, it's a new line
            if e.bbox[1] > heading_y_center:
                body_elems.append(e)
            else:
                heading_elems.append(e)
                # update center if needed, but usually first element represents the line well
        if body_elems:
            heading_elements = heading_elems
            overflow_elements = body_elems
    ...
    if span_to_block_idx is not None:
        ...
        for elem_idx, elem in enumerate(heading_elements):
        ...
        for group_idx, block_idx in enumerate(sorted_block_indices):
            ...
            subgroups = []
            if is_centered:
                subgroups = [[e] for e in group_elements]
            elif is_multicolumn and group_elements:
                font_sizes = [e.font_size for e in group_elements if e.font_size is not None]
                avg_font_size = sum(font_sizes) / len(font_sizes) if font_sizes else 12.0
                vg_thresh = max(5.0, avg_font_size * 0.5)
            ...
            for sg_idx, sg in enumerate(subgroups):
                if group_idx > 0 or sg_idx > 0:
                    ...
                    p.paragraph_format.right_indent = Inches(right_indent_from_margin_in)
                    p.paragraph_format.space_after = Pt(2)
                    if is_centered or block_type == "title":
                        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    elif block_type in ["figure_caption", "table_caption"]:
                        p.alignment = WD_ALIGN_PARAGRAPH.LEFT
                    elif block_type not in ["formula_caption"] and not style_name.startswith("Heading"):
        ...
        # Add overflow elements (e.g. [1] after "References" heading) as a new Normal paragraph
        if overflow_elements:
            p_overflow = docx_doc.add_paragraph(style="Normal")
            p_overflow.paragraph_format.left_indent = Inches(indent_from_margin_in)
            p_overflow.paragraph_format.right_indent = Inches(right_indent_from_margin_in)
        ...
        font_sizes = [e.font_size for e in heading_elements if e.font_size is not None]
        avg_font_size = sum(font_sizes) / len(font_sizes) if font_sizes else 12.0
        vertical_gap_threshold = avg_font_size * 1.5

        text_block_groups = []
        current_group = [heading_elements[0]]

        for i in range(1, len(heading_elements)):
            prev_elem = heading_elements[i - 1]
            curr_elem = heading_elements[i]
